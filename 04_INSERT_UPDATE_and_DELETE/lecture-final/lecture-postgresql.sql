...
SELECT * FROM countrylanguage WHERE countrycode = 'GBR'; -- just to check to see what language
INSERT INTO countrylanguage(countrycode, language, isofficial, percentage)
...
UPDATE country 
   SET capital = 3796 -- houston's code is 3796
   WHERE code = 'USA';

SELECT * FROM country WHERE code = 'USA';
-- 2. Update the capital of the USA to Washington DC and the head of state
SELECT * FROM city WHERE district = 'District of Columbia';
UPDATE country -- update the country table
   SET capital = (
      SELECT id FROM city WHERE name = 'Washington') -- SELECT returns the id from the city to be loaded into the country table in the capital col
   WHERE code = 'USA';
UPDATE country
...
START TRANSACTION; -- Same as BEGIN TRANSACTIOM
