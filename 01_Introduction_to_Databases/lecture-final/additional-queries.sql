...
SELECT name, population, gnp FROM country 
   WHERE gnp > 1000000 and population < 1000000000;
...
SELECT name, lifeexpectancy, region FROM country WHERE region ilike '%south%';  -- return any country with the word south in their region name
...
SELECT name, lifeexpectancy, region FROM country WHERE region ILIKE '%south%' AND lifeexpectancy IS NOT NULL;
